...
describe('isHiragana', () => {
    ...
    it('should narrow type to Hiragana', () => {
        ...
        if (syl.isHiragana(testString)) {
            expectTypeOf(testString).toEqualTypeOf<syl.Hiragana>()
        }
    })
})
...
describe('toHiragana', () => {
    ...
    it('should return a value of type Hiragana', () => {
        ...
        expectTypeOf(x).toEqualTypeOf<syl.Hiragana>()
    })
})

describe.concurrent('normalizeKana', () => {
    it('should strip dakuten from hiragana', () => {
        expect(syl.normalizeKana('が' as syl.Hiragana)).toBe('か')
        expect(syl.normalizeKana('できます' as syl.Hiragana)).toBe('てきます')
    })

    it('should strip handakuten from hiragana', () => {
        expect(syl.normalizeKana('ぱ' as syl.Hiragana)).toBe('は')
        expect(syl.normalizeKana('ぽかぽか' as syl.Hiragana)).toBe('ほかほか')
    })

    it('should do nothing to normal hiragana', () => {
        expect(syl.normalizeKana('あ' as syl.Hiragana)).toBe('あ')
        expect(syl.normalizeKana('わ' as syl.Hiragana)).toBe('わ')
        expect(syl.normalizeKana('ん' as syl.Hiragana)).toBe('ん')
    })
...
describe.concurrent('getColumn', () => {
    it('should throw an error for input longer than one kana', () => {
        expect(() => syl.getColumn('こんにちは' as syl.Hiragana)).toThrow('cannot get column for >1 kana')
    })

    it('should get corresponding vowel column', () => {
        expect(syl.getColumn('ぴ' as syl.Hiragana)).toBe('い')
        expect(syl.getColumn('あ' as syl.Hiragana)).toBe('あ')
        expect(syl.getColumn('ん' as syl.Hiragana)).toBe('ん')
    })
})

describe.concurrent('getFirst', () => {
    it('should get the first kana from hiragana string', () => {
        expect(syl.getFirst('ひらがな' as syl.Hiragana)).toBe('ひ')
        expect(syl.getFirst('わ' as syl.Hiragana)).toBe('わ')
    })
})

describe.concurrent('getLast', () => {
    it('should get last two kana for chouon syllables', () => {
        expect(syl.getLast('こーひー' as syl.Hiragana).sort).toEqual(['い', 'ひ'].sort)
        expect(syl.getLast('こーひー' as syl.Hiragana).sort).toEqual(['ひ', 'い'].sort)
    })

    it('should get last two kana for youon syllables', () => {
        expect(syl.getLast('きゃ' as syl.Hiragana).sort).toEqual(['き', 'や'].sort)
        expect(syl.getLast('きゃ' as syl.Hiragana).sort).toEqual(['や', 'き'].sort)
    })

    it('should get all chouon and youon kana in last syllable', () => {
        expect(syl.getLast('ねこきゃー' as syl.Hiragana).sort).toEqual(['き', 'や', 'あ'].sort)
    })

    it('should get single kana for regular syllables', () => {
        expect(syl.getLast('でも' as syl.Hiragana)).toEqual(['も'])
        expect(syl.getLast('じかん' as syl.Hiragana)).toEqual(['ん'])
    })
})

describe.concurrent('getCorresponding', () => {
    it('should get all n-kana for ん', () => {
        expect(syl.getCorresponding(['ん'] as syl.Hiragana[]).sort).toEqual(['な', 'に', 'ぬ', 'ね', 'の'].sort)
    })

    it('should get any corresponding dakuten, handakuten for normal kana', () => {
        expect(syl.getCorresponding(['わ'] as syl.Hiragana[])).toEqual(['わ'])
        expect(syl.getCorresponding(['か'] as syl.Hiragana[]).sort).toEqual(['か', 'が'].sort)
        expect(syl.getCorresponding(['は'] as syl.Hiragana[]).sort).toEqual(['は', 'ば', 'ぱ'].sort)
    })

    it('should get any corresponding normal kana, handakuten for dakuten', () => {
        expect(syl.getCorresponding(['で'] as syl.Hiragana[]).sort).toEqual(['で', 'て'].sort)
    })

    it('should get any corresponding normal kana, dakuten for handakuten', () => {
        expect(syl.getCorresponding(['ぷ'] as syl.Hiragana[]).sort).toEqual(['ふ', 'ぶ'].sort)
    })

    it('should get corresponding kana for all kana', () => {
        expect(syl.getCorresponding(['ん', 'ぽ'] as syl.Hiragana[]).sort).toEqual(['ほ', 'ぼ', 'ぽ', 'な', 'に', 'ぬ', 'ね', 'の'].sort)
    })
})